...
from .base_runner_new import EpochBasedRunner
...
from models.derived_yolox_backbone import FNA_YOLOX
from models.derived_yolof_backbone import FNA_Yolof
...
class NASRunner(EpochBasedRunner):
    def __init__(self, *args, **kwargs):
        ...
        if self.cfg.type == 'Retinanet':
            self.der_Net = lambda net_config: FNA_Retinanet(net_config, self.cfg.model.backbone.output_indices)
        ...
        elif self.cfg.type == 'YOLOX':
            self.der_Net = lambda net_config: FNA_YOLOX(net_config, self.cfg.model.backbone.output_indices)
        elif self.cfg.type == 'Yolof':
            self.der_Net = lambda net_config: FNA_Yolof(net_config, self.cfg.model.backbone.output_indices)
        else:
    ...
    def train_step_alter(self, data_loaders, **kwargs):
        ...
        while self._inner_iter < len(data_loaders[0]):
            for i, flow in enumerate(self.cfg.workflow):
                mode, steps = flow
                self.mode = 'train' if mode=='arch' else mode
                self.stage_mode = mode
                self.set_param_grad_state(self.stage_mode)

                if self.stage_mode=='arch' and self.search_stage == 0:
                    continue
                for _ in range(steps):
                    try:
                        ...
                        data_batch = data_loader_iters[i].next()
                    self.call_hook('before_train_iter', self.stage_mode)
                    outputs = self.batch_processor(
                        self.model, data_batch, mode=self.stage_mode, 
                    ...
                    if 'log_vars' in outputs:
                        self.log_buffer.update(outputs['log_vars'],
                                            outputs['num_samples'])
                    self.outputs = outputs
                    if self.stage_mode=='arch': # TODO: used for 5-step train
                        tmp = self._inner_iter
                        self._inner_iter -= 1
                        self.call_hook('after_train_iter', self.stage_mode)
                        self._inner_iter = tmp
                    elif self.stage_mode=='train':
                        self.call_hook('after_train_iter', self.stage_mode)
                    # if self.stage_mode=='train':
                        self._inner_iter += 1
                        self._iter += 1
    ...
    def init_nas_optimizer(self, optimizers):
        ...
        if isinstance(optimizers, dict):
            ...
            # print("arch_params_id: ", arch_params_id)
    ...
    def current_lr(self):
        ...
        if self.stage_mode is 'train':
            return [group['lr'] for group in self.optimizer.param_groups]
        elif self.stage_mode is 'arch':
    ...
    def register_training_hooks(self,
        ...
        # self.register_hook(OptimizerHook(**weight_optim_config))
        ...
        # self.register_arch_hook(ArchOptimizerHook(**arch_optim_config))
    ...
    def register_logger_hooks(self, log_config):
        ...
        for info in log_config['hooks']:
            if info.type == 'TextLoggerHook':
                ...
                logger_hook = NASTextLoggerHook(interval=log_interval)
            else:
