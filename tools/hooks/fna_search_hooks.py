...
from tools.utils import get_network_madds
...
class ModelInfoHook(Hook):
    ...
    def after_train_iter(self, runner):
        if self.iter % self.interval == 0 and runner.stage_mode=='arch':
    ...
    def comp_det_madds(self, runner, show_arch_params=True):
        ...
        derived_archs = runner.arch_gener.derive_archs(alphas, network_type=runner.type, logger=runner.logger)
...
class NASTextLoggerHook(TextLoggerHook):
    ...
    def _log_info(self, log_dict, runner):
        ...
        if runner.stage_mode in ['train', 'arch']:
            log_str = 'Epoch({})[{}][{}/{}] lr: {:.5f}, '.format(
                runner.stage_mode, log_dict['epoch'], log_dict['iter'], len(runner.data_loader),
                log_dict['lr'])
