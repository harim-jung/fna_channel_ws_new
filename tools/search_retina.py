...
def parse_args():
    # CUDA_VISIBLE_DEVICES=2,3,6,7 python -m torch.distributed.launch --nproc_per_node=4 ./tools/search_retina.py
    # CUDA_VISIBLE_DEVICES=6 python -m torch.distributed.launch --nproc_per_node=1 ./tools/search_retina.py
    
    # python -m torch.distributed.launch --nproc_per_node=8 ./tools/search_retina.py \
    # --config ./configs/fna_retinanet_fpn_search.py 

    parser = argparse.ArgumentParser(description='Train a detector')
    # Retinanet
    parser.add_argument('--model', default='retinanet', help='model name') # temp
    parser.add_argument('--config', default='./configs/fna_retinanet_fpn_search.py', help='train config file path')
...
def main():
    ...
    if args.work_dir is not None:
        if args.job_name is '':
            args.job_name = osp.join('output', args.model + '_adapt_check')
