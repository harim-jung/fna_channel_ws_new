...
class ArchGenerate_FNA(object):
    ...
    def derive_archs(self, alphas, network_type=None, logger=None):
        flat = lambda t: [x for sub in t for x in flat(sub)] if isinstance(t, Iterable) else [t]
        self.update_arch_params(alphas)
        def _parse(weights):
            # print("alpha len:", len(alphas))
            # print("num_layers len:", sum(self.num_layers))
            ...
            if network_type == 'Yolof':
                final_stages.append(['conv_2d_1x1'])
