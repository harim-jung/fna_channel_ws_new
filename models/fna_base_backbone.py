...
class BaseBackbone(nn.Module):
    def __init__(self, search_params, output_indices=[2, 3, 5, 7], pretrained=None):
    # def __init__(self, search_params, output_indices=[2, 3, 5, 7], init_config=None):
    ...
    def init_weights(self):#, pretrained=None):
        ...
        if self.pretrained is not None and self.pretrained.use_load:
            model_dict = remap_for_archadapt(self.pretrained.load_path, self.state_dict(), self.pretrained.seed_num_layers)
    ...
    def get_flops_list(self, input_shape):
        ...
        for block in self.blocks[2:]: # todo - double check for yolof
            layer_flops = []
